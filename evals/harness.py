...
OPENING_CHARS = 50
DEFAULT_DATASET = "Data/math_logic_samples.jsonl"
...
def basic_checks(df):
    ...
    if "thought" in df.columns:
        thought_lengths = df["thought"].fillna("").astype(str).str.len()
        print("\nThought length")
...
def monotone_check(df):
    """
    Checks for repetitive opening phrases in thought and response fields.
    """
    print("\n=== MONOTONE OPENINGS ===")
    ...
    for p in monotone_phrases:
        if 'thought' in df.columns:
            count = df['thought'].str.lower().str[:OPENING_CHARS].str.startswith(p).sum()
        else:
            count = 'no thought column'
        print(f"   '{p}': {count}")

    print("\nResponse openings:")
    for p in monotone_phrases:
        count = df['response'].str.lower().str[:OPENING_CHARS].str.startswith(p).sum()
        print(f"   '{p}': {count}")
