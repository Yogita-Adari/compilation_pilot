...
eda_columns = df[['instruction','thought','response']]        #new eda_Column that will scale across the datasets to unify evals without changing the code
# print(df.columns.tolist())      #show all columns
# print(df.shape)         #show shape - (r,c)

# print(df['thought'].iloc[0][:500])      #after running the code -- thought column seems more important than response
...
# print(df['thought'].str[:80].value_counts())        # check monotone -> we need to appears a lot
print(df['response'].str[:80].value_counts())